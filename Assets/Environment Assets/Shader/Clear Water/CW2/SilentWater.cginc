...
*/
		...
		void SilentWaterFunction (Input IN, inout SurfaceOutputStandard o)
		{
			...
			// fade by alpha?
			IN.color.a *= saturate(dot(IN.color.rgb, 1));

			// Needs raw UVs
			//float2 sinOffset = float2(sin(_Time.y + IN.waveUVs.y * UNITY_PI),cos(_Time.y + IN.waveUVs.x * UNITY_PI)) * 0.1;
			//IN.foamUVs += sinOffset.xyxy;
			//IN.waveUVs += sinOffset.xyxy;
			...
			// We use the reflection probe as a substitute for the lightmap in deep water,
			// as the lighting on the surface won't necessarily reach the deep end. 
			...
			float3 wave2 = UnpackScaleNormal(tex2D(_Wave2, wave1+IN.waveUVs.zw), _Wave2Strength);

			o.Normal = BlendNormalsPD( BlendNormalsPD(wave1, wave2), o.Normal);
			...
			foamFade = saturate(foamFade + foamFade - (1-surfaceFoam));
			foamFade *= _FoamDensity;

			surface.rgb = surface.rgb + surfaceFoam.rgb * foamFade;
			...
			interior += surfaceFoam * envSample * foamFade;
			interior *= edgeFade;
#endif
			...
			#else
			o.Albedo = surface.rgb * surfaceReduction * IN.color.a;
